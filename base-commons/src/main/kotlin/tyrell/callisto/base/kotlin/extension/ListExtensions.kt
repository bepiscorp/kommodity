...
public inline fun <T> List<T>.foreach(
 ...
 */
@ExperimentalLibraryApi
public inline fun <T> List<T>.foreachIndexed(
 ...
 */
@ExperimentalLibraryApi
public inline fun <T> List<T>.onEach(
    crossinline action: (element: T) -> Unit,
): List<T> = apply {
    for (index in indices) {
        val element = get(index)
...
/**
 * Performs the given [action] on each element, providing sequential index with the element,
 * and returns the list itself afterwards.
 *
 ...
 */
@ExperimentalLibraryApi
public inline fun <T> List<T>.onEachIndexed(
    crossinline action: (index: Int, element: T) -> Unit,
): List<T> = apply {
    for (index in indices) {
        val element = get(index)
